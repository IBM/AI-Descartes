...
OUTPUT_PATH_KEYMAERA = "../data/keymaera/output/"
INPUT_PATH_KEYMAERA = "../data/keymaera/input/"
...
def run_pipeline(variables, constants, data_points, axioms, interest_variable, input_formulas, precision, experiment_name):
    ...
    for formula in formulas_list:
        ...
        for measure in MEASURES:
            ...
            if measure == 'interval':
                file_name = experiment_name + '/formula' + formula_index + '__' + measure
                ...
                relerr_dependencies = {}
                # substitute this block with the block below for computing the exact value of the dependency errors
                for var_data in data_points[0]:
                    if var_data != interest_variable:
                        file_name = experiment_name + '/formula' + formula_index + '__' + measure + '_' + var_data
                        value2test = SENSITIVITY_ERROR + results['interval'][0]
                        convert_problem_keymaera(constants, variables, interest_variable, axioms, formula, 'dependencies', INPUT_PATH_KEYMAERA + file_name + '.kyx', data=data_points, var=var_data, relerr=value2test)
                ...
                for data_point in data_points:
                    ...
                    file_name = experiment_name + '/formula' + formula_index + '__' + measure + '_' + str(data_points.index(data_point))
                ...
                file_name = experiment_name + '/formula' + formula_index + '__' + measure
                convert_problem_keymaera(constants, variables, interest_variable, axioms, formula, 'derivation', INPUT_PATH_KEYMAERA + file_name + '.kyx' )
                result_derivation = call_keymaera(file_name)
                results['derivation'] = result_derivation
            elif measure == 'weak_derivation':
                file_name = experiment_name + '/formula' + formula_index + '__' + measure + "_constants"
                convert_problem_keymaera(constants, variables, interest_variable, axioms, formula, 'derivation_constants', INPUT_PATH_KEYMAERA + file_name + '.kyx')
                result_derivation = call_keymaera(file_name)
...
def binary_search(precision, constants, variables, interest_variable, axioms, formula, measure, file_name, data_points, var=None):
    ...
    convert_problem_keymaera(constants, variables, interest_variable, axioms, formula, measure, INPUT_PATH_KEYMAERA + file_name + '.kyx', data=data_points, var=var, relerr=upperbound)
    result = call_keymaera(file_name)
    ...
    while (upperbound - lowerbound) > precision:
        ...
        convert_problem_keymaera(constants, variables, interest_variable, axioms, formula, measure, INPUT_PATH_KEYMAERA + file_name + '.kyx', data=data_points, var=var, relerr=test_value)
        result = call_keymaera(file_name)
        if result == 'proved':
...
def call_keymaera(file_name, tool = 'mathematica'):
    ...
    :return: 'proved', 'not_proved', 'timeout_error', 'unknown_error', 'mathematica_error'
    '''
    # keymaerax_instance = kmx.KeYmaeraXTool()
    # result = keymaerax_instance.check(formula, tactic)
    formula_file = INPUT_PATH_KEYMAERA + file_name + '.kyx'
    formula_file_out = OUTPUT_PATH_KEYMAERA + file_name + '.kyp'
    # command = f"java -jar keymaerax/keymaerax.jar -launch -timeout 5 -prove {formula_file} -mathkernel /Applications/Mathematica.app/Contents/MacOS/MathKernel".split(" ")
    ...
    while (out not in ['proved', 'not_proved', 'timeout_error']) and num_bad_runs < TOLERANCE:
        ...
        out_file = OUTPUT_PATH_KEYMAERA + file_name + "_out.txt"
        # save the output
        new_file = open(out_file, "w+")
        new_file.write(keymaera_output)
        new_file.close()
        if (tool == 'mathematica' and (not 'Mathematica terminated' in keymaera_output)) or tool == 'z3':
...
def run_feynman():
    file_name = "feynman/Feynman_I_27_6"
    result_1 = call_keymaera(file_name)
    file_name = "feynman/Feynman_I_16_6"
    result_2 = call_keymaera(file_name)
    file_name = "feynman/Feynman_I_15_10"
...
def run_langmuir():
    file_name_list = ['langmuir/langmuir', 'langmuir/langmuir_2sites', 'langmuir/langmuir_const_1', 'langmuir/langmuir_const_2','langmuir/langmuir_const_3',
                      'langmuir/langmuir_const_4', 'langmuir/langmuir_const_5', 'langmuir/langmuir_const_6','langmuir/langmuir_const_7', 'langmuir/langmuir_const_8',
                      'langmuir/langmuir_const_9', 'langmuir/langmuir_const_10', 'langmuir/langmuir_const_11', 'langmuir/langmuir_const_12',
                      'langmuir/langmuir_const_13', 'langmuir/langmuir_const_14', 'langmuir/langmuir_2sites_const_1', 'langmuir/langmuir_2sites_const_2',
                      'langmuir/langmuir_2sites_const_3', 'langmuir/langmuir_2sites_const_4', 'langmuir/langmuir_2sites_const_5', 'langmuir/langmuir_2sites_const_6',
                      'langmuir/langmuir_2sites_const_7', 'langmuir/langmuir_2sites_const_8', 'langmuir/langmuir_2sites_const_9', 'langmuir/langmuir_2sites_const_10',
                      'langmuir/langmuir_2sites_const_11', 'langmuir/langmuir_2sites_const_12', 'langmuir/langmuir_2sites_const_13', 'langmuir/langmuir_2sites_const_14']
...
def run_kepler():
    file_name = "kepler/Kepler"
    result_1 = call_keymaera(file_name)
    print("Result for Kepler: " + str(result_1))
    file_name = "kepler/Kepler_const"
    result_2 = call_keymaera(file_name)
    print("Result for Kepler2: " + str(result_2))
    ...
    for formula in measure_vector:
        ...
        for measure in MEASURES:
            ...
            if measure == 'interval':
                ...
                new_file.write('\t- Results derivation: ' + str(results_week_derivation))
                new_file.write('\n')
...
class Test(unittest.TestCase):
    ...
    def test_keymaera1(self):
        formula_name = 'test'
        result = call_keymaera(formula_name, 'z3')
        self.assertEqual(result, 'proved')

    def test_keymaera2(self):
        formula_name = 'test_neg'
        result = call_keymaera(formula_name)
